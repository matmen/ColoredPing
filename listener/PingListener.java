			...
			this.main.protocolManager.addPacketListener(new PacketAdapter(
				...
				public void onPacketSending(PacketEvent event) {
					...
					try {
						...
						WrappedServerPing ping = (WrappedServerPing) event.getPacket().getServerPings().getValues()
								.get(0);
						...
						players.add(
								new WrappedGameProfile(UUID.randomUUID(), "        §0§lD§4§lB§6§lR §7| §c1.8        "));
						...
						if (ping.getPlayersOnline() < 1) {
							...
							for (Player all : org.bukkit.Bukkit.getOnlinePlayers()) {
								players.add(new WrappedGameProfile(all.getUniqueId().toString(),
										"§c" + all.getDisplayName()));
							}
						}
